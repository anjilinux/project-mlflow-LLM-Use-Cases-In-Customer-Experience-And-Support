...
# Use dummy embeddings to avoid OpenAI API calls
class DummyEmbeddings:
    def embed_documents(self, texts):
        # return zero vectors of fixed size
        return [[0.0] * 768 for _ in texts]

    def embed_query(self, text):
        return [0.0] * 768

print("🚀 Creating new FAISS vector store (dummy embeddings)...")

# Example text
texts = ["Hello world", "Testing vector store", "Jenkins CI/CD with dummy embeddings"]

# Split text into chunks
splitter = CharacterTextSplitter(chunk_size=10, chunk_overlap=0)
chunks = splitter.split_text(" ".join(texts))

# Create vector store with dummy embeddings
vectorstore = FAISS.from_texts(chunks, DummyEmbeddings())
